...
const cardObserver = new IntersectionObserver((entries) => {
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      if (!entry.target.dataset.animatedOnce) {
        entry.target.classList.add("fade-in", "scale-in", "in-view");
...
document
  .querySelectorAll(".testimonial-card, .feature-card")
  .forEach((card) => {
...
const imageObserver = new IntersectionObserver((entries) => {
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      entry.target.classList.add("fade-in", "in-view");
...
document
  .querySelectorAll("img:not(.no-anim):not(.hero-phone-img)")
  .forEach((img) => {
